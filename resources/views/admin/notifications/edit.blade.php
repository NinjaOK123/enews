...
<div class="p-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h3 class="text-2xl font-bold text-gray-900 dark:text-zinc-100 tracking-tight transition-colors">Chỉnh sửa thông báo: <span class="text-emerald-600">#{{ $notification->id }}</span></h3>
            <p class="text-sm text-gray-500 dark:text-zinc-400 mt-1 transition-colors">Cập nhật nội dung hoặc gửi lại thông báo đã tạo</p>
        </div>
    ...
    @if($notification->sent_at)
        <div class="mb-6 px-4 py-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-amber-800 dark:text-amber-400 rounded-xl flex items-center gap-3 animate-fade-in-up transition-colors">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-[0_2px_10px_rgb(0,0,0,0.04)] dark:shadow-none border border-gray-100 dark:border-zinc-800 overflow-hidden transition-colors">
        ...
        <form action="{{ route('admin.notifications.update', $notification) }}" method="POST">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        <label class="relative flex items-center gap-3 p-3 border-2 border-gray-100 dark:border-zinc-800 rounded-xl cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-zinc-800/50 has-[:checked]:border-emerald-500 dark:has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50/50 dark:has-[:checked]:bg-emerald-500/10 group">
                            <input type="checkbox" name="recipients[]" value="{{ $value }}" class="peer w-4 h-4 text-emerald-600 bg-gray-100 dark:bg-zinc-950 border-gray-300 dark:border-zinc-700 rounded focus:ring-emerald-500 dark:focus:ring-emerald-500/50 pointer-events-none transition-colors" {{ in_array($value, $currentRecipients) ? 'checked' : '' }}>
                            <div>
                                <div class="font-bold text-sm text-gray-900 dark:text-zinc-200 group-has-[:checked]:text-emerald-700 dark:group-has-[:checked]:text-emerald-400 transition-colors"><i class="bi {{ $info['icon'] }} me-1.5"></i>{{ $info['label'] }}</div>
                                <div class="text-[11px] text-gray-500 dark:text-zinc-500 transition-colors">{{ $info['sub'] }}</div>
                            </div>
                        </label>
                        @endforeach
                    </div>
            ...
            <div class="px-6 py-4 bg-gray-50/50 dark:bg-zinc-950/40 border-t border-gray-100 dark:border-zinc-800 flex flex-wrap items-center justify-end gap-3 rounded-b-2xl transition-colors">
                ...
                <button type="button" onclick="window.confirmSubmit(event, this, 'draft', 'Bạn có muốn LƯU THAY ĐỔI thông báo này không?')" class="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-zinc-800 border-2 border-emerald-500 dark:border-emerald-500 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-700 dark:hover:text-emerald-500 font-bold rounded-xl shadow-sm transition-all focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-500/30 focus:outline-none order-2">
