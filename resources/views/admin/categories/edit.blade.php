...
<div class="p-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h3 class="text-2xl font-bold text-white drop-shadow-md tracking-tight">Sửa chuyên mục</h3>
            <p class="text-sm text-emerald-50/90 font-medium mt-1">Cập nhật thông tin chuyên mục: <strong class="text-emerald-300">{{ $category->name }}</strong></p>
        </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="p-6 sm:p-8" x-data="{ isSubCategory: {{ $category->parent_id ? 'true' : 'false' }} }">
            ...
            <div class="mb-8">
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="w-5 h-5 text-emerald-600 bg-gray-100 border-gray-300 rounded focus:ring-emerald-500 cursor-pointer">
                    <div>
                        <span class="block text-sm font-semibold text-gray-700">Trạng thái công khai</span>
                        <span class="block text-xs text-gray-500 mt-0.5">Bật để cho phép chuyên mục hiển thị lên trang chủ</span>
                    </div>
                </label>
            </div>
