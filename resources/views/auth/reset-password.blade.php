...
@section('title', 'Đặt lại mật khẩu — E-News Đại học An Giang')
...
<div class="relative bg-white/10 backdrop-blur-xl border border-white/30 shadow-[0_8px_32px_rgba(0,0,0,0.3)] rounded-[1.5rem] p-8 sm:p-10 overflow-hidden">
    ...
    <div class="text-center mb-8">
        <div class="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-4 border border-white/30 shadow-inner">
            <i class="bi bi-shield-lock-fill text-3xl text-emerald-400 drop-shadow-md"></i>
        </div>
    ...
    <div class="bg-red-500/20 backdrop-blur-md border border-red-500/50 text-red-100 px-4 py-3 rounded-xl shadow-lg mb-6 text-sm flex items-start animate-fade-in-up">
        <i class="bi bi-exclamation-circle-fill mr-2 mt-0.5 text-red-400"></i>
        <div class="flex-1">
    ...
    <form method="POST" action="{{ route('password.update') }}" class="space-y-5" novalidate>
        ...
        <div>
            <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-[18px] flex items-center pointer-events-none">
                    <i class="bi bi-envelope-fill text-white/60 font-semibold text-lg transition-colors duration-300"></i>
                </div>
                <!-- Input glass: bg trong suốt nhẹ, viền trắng mờ -->
                <input type="email" name="email" value="{{ $email }}" required readonly
                       class="w-full bg-white/10 border border-white/20 text-white/70 placeholder-white/50 rounded-xl pl-12 pr-4 py-3.5 shadow-inner block cursor-not-allowed cursor-default">
            </div>
        </div>
        ...
        <div>
            <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-[18px] flex items-center pointer-events-none">
                    <i class="bi bi-key-fill text-white/60 group-focus-within:text-emerald-400 font-semibold text-lg transition-colors duration-300"></i>
                </div>
        ...
        <div>
            <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-[18px] flex items-center pointer-events-none">
                    <i class="bi bi-shield-check text-white/60 group-focus-within:text-emerald-400 font-semibold text-lg transition-colors duration-300"></i>
                </div>
        ...
        <button type="submit" class="relative w-full overflow-hidden group rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.6)] transition-all duration-300 mt-2">
            ...
            <div class="relative flex items-center justify-center px-4 py-[14px] text-[1.05rem] font-bold text-white tracking-wide">
                ...
                <i class="bi bi-check2-circle mt-[1px] ml-2 text-lg group-hover:scale-110 transition-transform duration-300"></i>
            </div>
        </button>
    </form>
